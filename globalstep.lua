...
local function process_step_every_catch_up(players, effect, dtime, now, elapsed)
	for i = 1, #players do
		...
		effect:on_step(player, value, dtime, now)
		...
		for _ = 1, math.floor(elapsed / effect.step_every) do
			...
			effect:on_step(player, effect:value(player), 0, now)
		end
...
local function process_step(players, effect, dtime, now)
	for i = 1, #players do
		local player = players[i]
		local meta = player:get_meta()
		local remainings = effect:_get_remainings(meta)
			...
			if effect.step_every then
				...
				if elapsed >= effect.step_every then
					if effect.step_catchup then
						elapsed = process_step_every_catch_up(players, effect, dtime, now, elapsed)
					else
						process_step(players, effect, dtime, now)
						elapsed = 0
					end
				end
